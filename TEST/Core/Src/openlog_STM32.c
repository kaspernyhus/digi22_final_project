...
void openlogCmdMode()
{
    ...
    HAL_UART_Transmit(&huart3, (uint8_t*)cmdBuf, 3, HAL_MAX_DELAY);
    HAL_Delay(30);
}
...
void openlogWriteStr(char* string)
{
	...
	char* p = string;
	while(*p != '\0') {
		HAL_UART_Transmit(&huart3, (uint8_t*)p, 1, HAL_MAX_DELAY);
		p++;
		HAL_Delay(5);
	}
	HAL_UART_Transmit(&huart3, (uint8_t*)crBuf, 1, HAL_MAX_DELAY);
	HAL_Delay(5);
}
